	...
	for (i = 0; i < CONFIG_NET_MAX_ROUTERS; i++) {
		...
		if (IS_ENABLED(CONFIG_NET_IPV6) && family == AF_INET6) {
			...
			NET_DBG("interface %p router %s lifetime %u default %d "
				"added",
				iface, net_sprint_ipv6_addr(addr), lifetime, routers[i].is_default);
...
struct net_if_addr *net_if_ipv6_addr_lookup_raw(const uint8_t *addr, struct net_if **ret)
{
	...
	STRUCT_SECTION_FOREACH(net_if, iface) {
		...
		ARRAY_FOR_EACH(ipv6->unicast, i) {
			...
			if (net_ipv6_is_prefix(addr, ipv6->unicast[i].address.in6_addr.s6_addr,
					       128)) {
...
struct net_if_addr *net_if_ipv6_addr_lookup_by_iface_raw(struct net_if *iface, const uint8_t *addr)
{
	struct net_if_addr *ifaddr = NULL;
	struct net_if_ipv6 *ipv6;
	...
	ARRAY_FOR_EACH(ipv6->unicast, i) {
		...
		if (net_ipv6_is_prefix(addr, ipv6->unicast[i].address.in6_addr.s6_addr, 128)) {
			ifaddr = &ipv6->unicast[i];
			goto out;
		}
	}
...
struct net_if_mcast_addr *net_if_ipv6_maddr_lookup_raw(const uint8_t *maddr, struct net_if **ret)
{
	struct net_if_mcast_addr *ifmaddr = NULL;
	...
	STRUCT_SECTION_FOREACH(net_if, iface) {
		...
		ARRAY_FOR_EACH(ipv6->mcast, i) {
			...
			if (net_ipv6_is_prefix(maddr, ipv6->mcast[i].address.in6_addr.s6_addr,
					       128)) {
				if (ret) {
					*ret = iface;
				}
...
static bool ipv4_is_broadcast_address(struct net_if *iface, const uint8_t *addr)
{
	struct net_if_ipv4 *ipv4;
	bool ret = false;
...
bool net_if_ipv4_is_addr_bcast_raw(struct net_if *iface, const uint8_t *addr)
{
	bool ret = false;
...
struct net_if_addr *net_if_ipv4_addr_lookup_raw(const uint8_t *addr, struct net_if **ret)
{
	struct net_if_addr *ifaddr = NULL;
